...
// Several of the following functions have been taken directly from
// the go standard library then modified.
...
func parseRange(s string, size int64) ([]httpRange, error) {
	...
	for _, ra := range strings.Split(s[len(b):], ",") {
		...
		if start == "" {
			...
			r.start = i
			if end == "" {
				// If no end is specified, range extends to (default
				// transfer + start) bytes or the end of the file.
				end := r.start + int64(defaultTransfer)
